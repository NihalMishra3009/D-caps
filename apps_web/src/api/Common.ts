      ...
      const segments = [
        {
          ...
          from: { lat: wh.latitude, long: wh.longitude },
          to: assignedLocs[0] ? { lat: assignedLocs[0].latitude, long: assignedLocs[0].longitude } : { lat: wh.latitude, long: wh.longitude },
        },
        ...assignedLocs.map((loc: any, lIdx: number) => ({
          ...
          segmentType: 'TO_DESTINATION',
          from: { lat: loc.latitude, long: loc.longitude },
          to: assignedLocs[lIdx + 1]
            ? { lat: assignedLocs[lIdx + 1].latitude, long: assignedLocs[lIdx + 1].longitude }
            : { lat: wh.latitude, long: wh.longitude },
        })),
      ]
